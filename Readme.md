...
- オプションの詳細
  ...
  - `Toggle` - 出力キーの押しっぱなし状態が入力キーを押す度切り替わるようにします。
  - `Repeat` - 出力キーを設定した間隔でリピートする状態が入力キーを押す度切り替わるようにします。
  - `Hold` - 出力キーを設定した時間だけ自動でホールドするようにします。
  - `Run as admin` - 管理者権限でコマンドを実行するようにします。

#### ホットキーの仕様

- キーの優先度はアクティブウィンドウ→アクティブでないウィンドウ→すべてのウィンドウの順番です。
...
- メインウィンドウの`Profile`タブにある`Create`ボタンを押すと表示される作成ウィンドウにプロファイル名を入力すると新規プロファイルを作成できます。
