...
- 入力設定項目のウィンドウ名入力欄にウィンドウ名か実行ファイル名が指定されていると、そのウィンドウに対して機能するホットキーを作成出来ます。  
...
- `入力キー`+`ウィンドウ名`+`Direct sendの有無`の組み合わせが一致していない限り、同じ入力キーのホットキーは複数存在出来ます。
...
#### リンクデータの作成･編集の詳細
- 作成･編集ウィンドウにはプロファイル名選択欄とウィンドウ名入力欄とモード選択欄があり、  
  指定ウィンドウが指定モード時に指定プロファイルを読み込むという情報を設定し、プロファイルとウィンドウを関連付けることが出来ます。
- ウィンドウ名入力欄にはウィンドウ名か実行ファイル名のどちらかを指定出来ます。  
