...
- モードの詳細
  - `Active`  
    指定したウィンドウがアクティブな時に読み込むようにします。
  - `Exist`  
    指定したウィンドウが非アクティブな時でもアクティブウィンドウに関連付けられた他のプロファイルがなければ読み込むようにします。
  - `Absolute`  
    指定したウィンドウが存在する時、アクティブウィンドウに関連付けられた他のプロファイルがあっても読み込むようにします。  
    アクティブウィンドウに関連付けられた他のプロファイルが存在した場合は、`Absolute`に設定されたプロファイルと重複していない部分のみ読み込みます。
